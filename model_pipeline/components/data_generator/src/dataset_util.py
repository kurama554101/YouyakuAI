...
class LivedoorDatasetUtil:
    ...
    def download_and_get_all_data(self) -> list:
        ...
        archive_path = os.path.join(
            self.__base_folder, os.path.basename(self.__archive_url)
        )
        urllib.request.urlretrieve(self.__archive_url, archive_path)

        # データを取得
        all_data = self.get_all_data_from_archive_file(
    ...
    def get_all_data_from_archive_file(
        ...
        with tarfile.open(archive_path) as archive_file:
            for archive_item in archive_file:
                for i, genre in enumerate(self.__target_genres):
                    if (
                        genre in archive_item.name
                        and archive_item.name.endswith(".txt")
                    ):
            ...
            for i, genre_files in enumerate(genre_files_list):
                for name in genre_files:
                    ...
                    if len(title) > 0 and len(body) > 0:
                        df = df.append(
                            pd.Series([title, body, i], index=df.columns),
                            ignore_index=True,
                        )
            return df

    def split_data(
        self, all_data: pd.DataFrame, random_seed: int = 1000
    ) -> dict:
        train_list = []
        val_list = []
        test_list = []
        ...
        for i, row in all_dara_shuffle.iterrows():
            if i < self.__train_ratio / total_ratio * data_size:
                ...
                i
                < (self.__train_ratio + self.__val_ratio)
                / total_ratio
                * data_size
            ):
    ...
    def write_files_from_data(
        ...
        total_ratio = self.__train_ratio + self.__val_ratio + self.__test_ratio
        with open(train_file_path, "w", encoding="utf-8") as f_train, open(
            val_file_path, "w", encoding="utf-8"
        ) as f_dev, open(test_file_path, "w", encoding="utf-8") as f_test:
    ...
    def __read_title_body(self, file):
        ...
        body = normalize_text(
            " ".join(
                [line.decode("utf-8").strip() for line in file.readlines()]
            )
        )
        return title, body
