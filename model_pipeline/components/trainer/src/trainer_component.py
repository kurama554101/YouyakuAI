...
class T5FineTuner(pl.LightningModule):
    ...
    @abstractmethod
    def get_dataset(
        self, file_path: str = None, data_frame: pd.DataFrame = None
    ):
        pass
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            train_file_path = get_dataset_path(
                dataset_params=self._dataset_params, key="train_file_path"
            )
            train_df = get_dataframe(
                dataset_params=self._dataset_params, key="train_dataset"
            )
            self.train_dataset = self.get_dataset(
                file_path=train_file_path, data_frame=train_df
            )

            val_file_path = get_dataset_path(
                dataset_params=self._dataset_params, key="val_file_path"
            )
            val_df = get_dataframe(
                dataset_params=self._dataset_params, key="val_dataset"
            )
            self.val_dataset = self.get_dataset(
                file_path=val_file_path, data_frame=val_df
            )
            ...
            self.t_total = (
                (
                    ...
                    // (
                        self._hparams["train_batch_size"]
                        * max(1, self._hparams["n_gpu"])
                    )
                )
