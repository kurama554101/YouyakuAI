...
def save_model(model: T5FineTunerWithLivedoorDataset, out_dest: OutputDestinations):
...
def train(parameters: dict,
          ...
          test_dataset: pd.DataFrame) -> T5FineTunerWithLivedoorDataset:
    dataset_params = dict(
        train_dataset=train_dataset,
        val_dataset=val_dataset,
        test_dataset=test_dataset
...
def create_args_from_parameters(parameters: dict,
...
if __name__ == "__main__":
    ...
    model.save(model=model,
               model_dir=artifacts.output_destinations.trained_model_dir)
