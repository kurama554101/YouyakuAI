...
def _trainer_op(train_data_path: str,
                val_data_path: str,
                test_data_path: str,
                suffix: str,
                parameters: str) -> kfp.dsl.ContainerOp:
    name = "trainer"
    component_spec = get_component_spec(name)
...
def create_pipeline_func():
    ...
    pipeline_root = os.path.join(os.path.dirname(__file__), deploy_dict["local"]["pipeline_root"])
    ...
    def kfp_youyakuai_pipeline(suffix: str = suffix, parameters: str = params):
        ...
        trainer = _trainer_op(
            ...
            test_data_path=data_generator.outputs[GeneratedData.TestData.value],
            suffix=suffix,
            parameters=parameters
        )
    return kfp_youyakuai_pipeline, {"pipeline_root": pipeline_root, "suffix": suffix, "parameters": params}
...
def compile_pipeline(file_path: str):
    ...
    if ext == ".yaml":
        ...
        raise NotImplementedError("{} file type is not implemented!".format(ext))
...
if __name__ == "__main__":
    ...
    if args.run_pipeline:
        ...
        run_pipeline(pipeline_func=pipeline_func, pipeline_arg_dict=pipeline_arg_dict)
