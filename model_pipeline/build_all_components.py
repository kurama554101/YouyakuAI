    ...
    def value_of(cls, deploy_type_str: str):
        for e in DeployType:
            if e.value == deploy_type_str:
                return e
        raise ValueError(
            "{} is not exists in DeployType!".format(deploy_type_str)
        )
...
def main(deploy_type: DeployType):
    ...
    for name in component_list:
        build_and_deploy_image(
            base_dir=base_dir, name=name, deploy_type=deploy_type
        )
...
def build_and_deploy_image(base_dir: str, name: str, deploy_type: DeployType):
    ...
    command = "docker build --target production -t {} {}".format(
        base_image_name, component_dir
    )
...
def get_args():
    ...
    parser.add_argument(
        "--deploy_type",
        default="local",
        help="deploy type is decision of saving place for docker image. value type is 'local', 'docker_hub'.",
    )
