...
def create_kafka_topics_if_needed(
    topics: List[str], client_id: str, config: QueueConfig
):
    client = KafkaAdminClient(
        bootstrap_servers=config.get_url(), client_id=client_id
    )
    ...
    try:
        ...
        logger.error(
            "{} topic is already exists! detail is {}".format(topics, e)
        )
