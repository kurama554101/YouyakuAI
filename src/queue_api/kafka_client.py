...
import asyncio
from queue_client import AbstractQueueInitializer, AbstractQueueConsumer, AbstractQueueProducer, QueueConfig, QueueError, _QueueInternalResult
from kafka_helper import exist_kafka_topic, create_kafka_topics_if_needed, get_kafka_topics
...
class KafkaQueueInitializer(AbstractQueueInitializer):
    ...
    def initialize(self):
        ...
        self._logger.info("create new kafka topic if needed. name is {}".format(topic_name))
...
class KafkaQueueProducer(AbstractQueueProducer):
    def produce(self, messages:list):
        ...
        if internal_result._e is not None:
            raise QueueError("QueueProducer Error! error detail is {}".format(internal_result._e))
    ...
    async def __produce(self, loop, messages:list) -> _QueueInternalResult:
        ...
        try:
            producer = AIOKafkaProducer(loop=loop, bootstrap_servers=self._config.get_url(),
...
class KafkaQueueConsumer(AbstractQueueConsumer):
    def consume(self) -> list:
        ...
        if internal_result._e is not None:
            raise QueueError("QueueConsumer Error! error detail is {}".format(internal_result._e))
    ...
    async def __consume(self, loop) -> _QueueInternalResult:
        ...
        try:
            consumer = AIOKafkaConsumer(self._config.optional_param["topic_name"],
                                        loop=loop,
