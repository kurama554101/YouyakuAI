...
def main():
    ...
    engine = create_engine("mysql+mysqlconnector://{}:{}@{}:{}/{}?charset={}".format(os.environ.get("DB_USERNAME"),
    ...
    with Session() as session:
        body_info = BodyInfo(body="てすとです", created_at=datetime(year=2021, month=1, day=1, hour=10, minute=0))
    ...
    if body_id is None:
        ...
        return
    with Session() as session:
        session.add(SummarizeResult(body_id=body_id, predicted_text="予測です", label_text="外れます"))
        session.commit()

def main2():
    config = DBConfig(host=os.environ.get("DB_HOST"),
                      port=os.environ.get("DB_PORT"),
                      username=os.environ.get("DB_USERNAME"),
                      password=os.environ.get("DB_PASSWORD"),
                      db_name=os.environ.get("DB_NAME"))
    logger = LoggerFactory.get_logger(logger_type="print", logger_name="debug_db")
    db_instance = DBFactory.get_db_instance(db_config=config, log_instance=logger)
    ...
    result_list = [SummarizeResult(body_id=id, predicted_text="予測です", label_text="外れます") for id in ids]
