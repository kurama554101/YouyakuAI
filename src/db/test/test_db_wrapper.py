...
class TestDBWrapper(unittest.TestCase):
    ...
    def test_insert_body_infos(self):
        ...
        ids = self.db_instance.insert_body_infos(body_infos=expected_body_infos)

        # BodyInfoの取得を行い、データが正しいかを検証
        actual_body_infos = self.db_instance.fetch_body_infos()
        for expected_body_info, expected_body_text in zip(expected_body_infos, expected_body_texts):
            actual_body_info = [body_info for body_info in actual_body_infos if body_info.id == expected_body_info.id][0]
    ...
    def test_insert_summarizer_results(self):
        ...
        expected_predicted_texts = [
            "予測です",
            "予測です2"
        ]
        expected_summarizer_results = [
            SummarizeResult(body_id=ids[0], inference_status=InferenceStatus.complete.value, predicted_text=expected_predicted_texts[0], label_text=expected_predicted_texts[0]),
            SummarizeResult(body_id=ids[1], inference_status=InferenceStatus.complete.value, predicted_text=expected_predicted_texts[1], label_text=expected_predicted_texts[1])
        ]
        result_ids = self.db_instance.insert_summarize_results(result_infos=expected_summarizer_results)

        # SummarizerResultの取得を行い、データが正しいかを検証
        actual_results = self.db_instance.fetch_summarize_results()
        for expected_result, expected_predicted_text in zip(expected_summarizer_results, expected_predicted_texts):
            actual_result = [result for result in actual_results if result.id == expected_result.id][0]
            self.assertEqual(expected_result, actual_result)
            self.assertEqual(expected_predicted_text, actual_result.get_predicted_text())
            self.assertEqual(expected_predicted_text, actual_result.get_label_text())
    ...
    def test_fetch_summarize_result_by_id(self):
        ...
        actual_result = self.db_instance.fetch_summarize_result_by_id(result_id=result_ids[0])
    ...
    def test_fetch_summarize_job_info(self):
        ...
        actual_job_info = self.db_instance.fetch_summarize_job_info(job_id=expected_job_info.job_id)
