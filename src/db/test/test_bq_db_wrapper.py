...
class TestBigQueryDBWrapper(unittest.TestCase):
    ...
    def test_insert_body_infos(self):
        ...
        for i, id in enumerate(ids):
            actual_info = self.db_instance.fetch_body_info_by_id(
                body_info_id=id
            )
    ...
    def test_insert_summarize_results(self):
        result_infos = [
            SummarizeResult(
                body_id=uuid.uuid4(),
                inference_status=0,
                predicted_text="これはテストです1",
                label_text="間違いです1",
            ),
            SummarizeResult(
                body_id=uuid.uuid4(),
                inference_status=0,
                predicted_text="これはテストです2",
                label_text="間違いです2",
            ),
        ]

        # insert処理の実行
        ids = self.db_instance.insert_summarize_results(
        ...
        # 追加したデータが一致しているかを確認
        for i, id in enumerate(ids):
            actual_info = self.db_instance.fetch_summarize_result_by_id(
                result_id=id
            )
            self.assertIsNotNone(actual_info)
            self.assertEqual(result_infos[i].body_id, actual_info.body_id)
            self.assertEqual(
                result_infos[i].inference_status, actual_info.inference_status
            )
            self.assertEqual(
                result_infos[i].label_text, actual_info.label_text
            )
            self.assertEqual(
                result_infos[i].predicted_text, actual_info.predicted_text
            )
    ...
    def test_insert_summarize_job_infos_with_result_id_is_none(self):
        job_infos = [SummarizeJobInfo(job_id=uuid.uuid4(), result_id=None)]
    ...
    def test_insert_summarize_job_logs(self):
        job_logs = [SummarizeJobLog(job_id=uuid.uuid4())]
        ...
        # 追加したデータが取得可能かを確認
        for id in ids:
            actual_log = self.db_instance.fetch_summarize_job_log_by_id(
                job_id=id
            )
    ...
    @unittest.skip(
        "update method is skip because of bigquery limitation. detail is 'https://cloud.google.com/bigquery/docs/reference/standard-sql/data-manipulation-language#limitations'"
    )
