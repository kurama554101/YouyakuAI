...
class LoggerFactory():
    @classmethod
    def get_logger(cls, logger_type:str, logger_name:str, boto3_session=None):
        if logger_type == "print":
            return PrintLogger(logger_name=logger_name)
        elif logger_type == "cloudwatch":
            return CloudWatchLogger(boto3_session=boto3_session, logger_name=logger_name)
        else:
            raise NotImplementedError("{} logger type is not implemented!".format(logger_type))
...
class CloudWatchLogger(AbstractLogger):
    def __init__(self, boto3_session, logger_name:str) -> None:
        ...
        self._logger.addHandler(watchtower.CloudWatchLogHandler(boto3_session=boto3_session))
...
class PrintLogger(AbstractLogger):
    ...
    def info(self, message):
        print("INFO: " + self._logger_name + " : " + self._log_message(message))

    def error(self, message):
        print("ERROR: " + self._logger_name + " : " + self._log_message(message))

    def error_detail(self, err:Exception):
        err_txt, err_detail_txt = self._create_error_detail_txt(err)
        self.error(err_txt)
        print("ERROR Detail: " + self._logger_name + " : " + self._log_message(err_detail_txt))
