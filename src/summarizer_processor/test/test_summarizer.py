...
class TestSuammarizer(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.logger = LoggerFactory.get_logger(
            logger_type="print", logger_name="test_summarizer"
        )
        ...
        self.queue = QueueConsumerForTest(
...
class DBForTest(AbstractDB):
    ...
    def update_summarize_results(
        ...
        for result_info in result_infos:
            target_result_info_indexes = [
                key
                for key, info in enumerate(self.__result_infos)
                if result_info.id == info.id
            ]
    ...
    def update_summarize_job_info(
        ...
        for job_info in job_infos:
            target_job_info_indexes = [
                key
                for key, info in enumerate(self.__job_infos)
                if job_info.job_id == info.job_id
            ]
