...
class ResponseInferenceStatusDetail(str, Enum):
    ...
    job_is_not_found = "inference job is not found. please try to request inference again"
...
class ResponseInferenceStatus(Enum):
    complete_job     = (ResponseInferenceStatusCode.complete_job, ResponseInferenceStatusDetail.complete_job)
    in_progress_job  = (ResponseInferenceStatusCode.in_progress_job, ResponseInferenceStatusDetail.in_progress_job)
    job_is_not_found = (ResponseInferenceStatusCode.job_is_not_found, ResponseInferenceStatusDetail.job_is_not_found)
    complete_job_but_not_found_inference_result = (ResponseInferenceStatusCode.complete_job_but_not_found_inference_result,
...
class ResponseSetCorrectedResult(Enum):
    ...
    inference_job_is_not_found = (2, "inference job is not found. please try to request inference again.")
    complete_inference_job_but_not_found_inference_result = (3, "inference job is completed! but inference result is not found because of unknown error.")
    failed_to_update_label = (4, "fail to update corrected text because of db error")
...
class SummarizerApi:
    ...
    def create_app(self) -> FastAPI:
        ...
        async def set_correct_summarize_result(correct_data: CorrectInputData):
            return await self.__set_correct_summarize_result(correct_data=correct_data)
    ...
    async def __get_summarize_result(self, job_id: str):
        ...
        # ジョブが完了している場合
        result = self.__db_instance.fetch_summarize_result_by_id(result_id=job_info.result_id)
        if result is None:
            # 何らかの理由で、ジョブ情報はあるが、推論結果がない場合はエラーとして返す
            response_status = ResponseInferenceStatus.complete_job_but_not_found_inference_result
            return make_response(response_status=response_status)
    ...
    async def __set_correct_summarize_result(self, correct_data: CorrectInputData):
        ...
        # ジョブが見つからない場合
        if job_info is None:
            response_status = ResponseSetCorrectedResult.inference_job_is_not_found
            return make_response(response_status=response_status)

        # ジョブが完了していない場合
        if job_info.result_id is None:
            response_status = ResponseSetCorrectedResult.inference_job_is_not_completed
            return make_response(response_status=response_status)

        # ジョブが完了している場合
        try:
            self.__db_instance.update_label_text_of_result_by_id(result_id=job_info.result_id, label_text=correct_data.corrected_text)
        except Exception as e:
            self.__logger.error("update corrected text! error detail is {}".format(e))
            response_status = ResponseSetCorrectedResult.failed_to_update_label
            return make_response(response_status=response_status)
        return make_response(response_status=ResponseSetCorrectedResult.complete_job)
